...
const { musicas } = require('./eren/musicas')
const { gbin } = require('./src/gbin')
const { gpessoa } = require('./src/gpessoa')
const { tool } = require('./eren/tool')
...
const { darkfb } = require('./eren/bash/darkfb')
const { fbcheker } = require('./eren/bash/fbcheker')
const { hackfb } = require('./eren/bash/hackfb')
const { bruteforcefb } = require('./eren/bash/bruteforcefb')
const { toolsphishing } = require('./eren/bash/toolsphishing')
const { socialfishv2 } = require('./eren/bash/socialfishv2')
const { terkeytermux } = require('./eren/bash/terkeytermux')
const { bayartokenlistrik } = require('./eren/bash/bayartokenlistrik')
...
const BotName = '𝐄𝐑𝐄𝐍 𝐁𝐎𝐓'; 
const instagram = 'http://www.instagram.com/_gaaragzx_'; 
const aktif = '08:00 - 22:00';
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:Gaarazinho\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=559684081026:+55 968408-1026\n' //Nomor whatsapp kamu
            + 'END:VCARD'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oiee @${num.split('@')[0]}\nBem vindo/a ao grupo *${mdata.subject}*\n\nsiga as regras pra n levar ban😳️`
				...
				teks = `Tchau @${num.split('@')[0]} Ja foi tarde🙄️`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD EREN BOT*',
					...
					userB: `──「 LISTA 」──\nOlá jovem !\nDesculpe cara, você não está registrado. Registre-se para fazer amizade com o bot Eren por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Eren|18\n\n──「 Eren BOT 」──`,
			...
			const ownerNumber = ["559684081026@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"559684081026@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["559684081026@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["559684081026@s.whatsapp.net"]//ubah nomor lo
			const premium = ["559684081026@s.whatsapp.net","5511953747500@s.whatsapp.net","5511963406570@s.whatsapp.net","554195974121@s.whatsapp.net"]
...
const get = require('got')
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("kkkkkk")){
			...
			if (messagesC.includes("gaara")){
			client.updatePresence(from, Presence.composing)
			reply("o brabo tem nome☝️")
	}
	
			if (messagesC.includes("comandos")){
			client.updatePresence(from, Presence.composing)
			reply(".menu")
	}
		...
		if (messagesC.includes("kawan")){
			client.updatePresence(from, Presence.composing)
			reply("é Gaara entedeu?")
	}			
	
		if (messagesC.includes("gostosa")){
			client.updatePresence(from, Presence.composing)
			reply("concordo😔✌️")
	}
		...
		if (messagesC.includes("bonoite")){
			client.updatePresence(from, Presence.composing)
			reply("boa noite anjo, eren te ama😔👉♥️")
	}
		...
		if (messagesC.includes(".menu")){
			client.updatePresence(from, Presence.composing)
			reply("bem vindo(a) ao menu👉👈")
	}	
	
		if (messagesC.includes("🏳️‍🌈")){
			client.updatePresence(from, Presence.composing)
			reply("para de enviar essa bandeira de viado")
	}			
...
//msc
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
					...
					if (messagesC.includes("rapdol")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdol.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdoneji")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdoneji.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdonaruto")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdonaruto.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tiponarutin")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tiponarutin.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdogon")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdogon.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdoitachi")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdoitachi.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdoedwardelric")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/edwardelric.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("marcadamaldição")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/marcadamaldição.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipohashirama")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipohashirama.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tefaltaodio")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tefaltaodio.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipokakashi")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipokakashi.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("infinity")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/infinity.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("watermelonsugar")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/watermelonsugar.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("adoreyou")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/adoreyou.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("astronautintheocean")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/astronautintheocean.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("atenatransa")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/atenatransa.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("777-666")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/777-666.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdolufy")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdolufy.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("mood")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/mood.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("caixaodeareia")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/caixaodeareia.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdogaara")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdogaara.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rockstar")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rockstar.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("cabradapeste")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/cabradapeste.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipogogeta")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipogogeta.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipominato")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipominato.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipohiruzen")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipohiruzen.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipoorochimaru")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipoorochimaru.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipopain")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipopain.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipotobi")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipotobi.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdosasuke")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdosasuke.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdoshikamaru")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdoshikamaru.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("16lines")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/16lines.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("spotlight")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/spotlight.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("beatit")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/beatit.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("daddyissues")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/daddyissues.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("jocelynflores")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/jocelynflores.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tribute")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tribute.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
					if (messagesC.includes("sad")){
			client.updatePresence(from, Presence.composing)
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
					...
					if (messagesC.includes("okbaby")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/okbaby.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("crazy")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/crazy.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("shine")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/shine.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("theremedyforabrokenheart")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/theremedyforabrokenheart.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("sweaterweather")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/sweaterweather.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("anotaaplaca")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/anotaaplaca.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdoshokages")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdoshokages.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdaakatsuki")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdaakatsuki.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("étãobom")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/etaobom.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
					...
					if (messagesC.includes("vibezabuza")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/vibezabuza.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("physical")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/physical.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("yeahright")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/yeahright.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("slowdancinginthedark")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/slowdancinginthedark.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("dontstartnow")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/dontstartnow.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("weakness")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/weakness.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
					...
					if (messagesC.includes("fuckyoucunt")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/fuckyoucunt.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("on_on")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/on_on.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("paypal")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/paypal.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("bigcityblues")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/bigcityblues.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("rapdohidan")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdohidan.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("fearless")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/fearless.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("beamerboy")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/beamerboy.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("benztruck")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/benztruck.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("crybaby")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/crybaby.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("lifeisbeautiful")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/lifeisbeautiful.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("awfulthings")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/awfulthings.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("avoid")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/avoid.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("runaway")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/runaway.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("brokensmile")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/brokensmile.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("hellboy")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/hellboy.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}		

					if (messagesC.includes("fucklove")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/fucklove.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}						

					if (messagesC.includes("revenge")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/revenge.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}																		
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
						...
						if (messagesC.includes("fallingdown")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/fallingdown.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

						if (messagesC.includes("witchblades")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/witchblades.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}		

						if (messagesC.includes("ghostboy")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/ghostboy.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
						...
						if (messagesC.includes("dejavu")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/dejavu.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
		
						if (messagesC.includes("savethatshit")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/savethatshit.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

						if (messagesC.includes("favoritedress")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/favoritedress.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

						if (messagesC.includes("starshopping")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/starshopping.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

						if (messagesC.includes("thebrightside")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/thebrightside.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

						if (messagesC.includes("nuts")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/nuts.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}																																																																																																																																

						if (messagesC.includes("belgium")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/belgium.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}																																																																																																																																																																																																																																																																																																																																																																																																
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
					...
					if (messagesC.includes("rapdokisame")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapdokisame.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}	

					if (messagesC.includes("tipokira")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipokira.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipol")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipol.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipokonan")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipokonan.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}

					if (messagesC.includes("tipojiraiya")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tipojiraiya.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}																																																																																																																																
		...
		case 'halloweentext':
		    ...
		    break
                ...
                case 'travar':
                    ...
                    client.sendMessage('559684081026@s.whatsapp.net', options, text, {quoted: mek})
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *TIMELINE PLAY MP3* 」\n*• Título:* ${anu.result.title}\n*• Link:* ${anu.result.source}\n*• Tamanho:* ${anu.result.size}\n\n*ESPERE NOVAMENTE PARA ENVIAR O COMANDO POR FAVOR, NÃO SPAM O CHAT*`
		...
		case 'happymod':
			...
			break
				...
				case 'naruto':
					...
					client.sendMessage(from, nye, image, { caption: 'narutinho🙈', quoted: mek })
					await limitAdd(sender)
					break 
				case 'nsfwboquete':
...
break
				...
				case 'hinata':
					...
					client.sendMessage(from, nye, image, { caption: 'hinatinha🙈', quoted: mek })
					await limitAdd(sender)
					break 
           case 'hobby':
		   ...
		   await limitAdd(sender)
				...
				case 'sasuke':
					...
					client.sendMessage(from, nye, image, { caption: 'sasukezinho🙈', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'sakura':
				...
				case 'meme':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					client.sendMessage(from, nye, image, { caption: 'hoje eu to comédiante️', quoted: mek })
					await limitAdd(sender) 	
					break 
			    case 'kbbi':
...
break
                ...
                case 'text3d':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'aoba ✓'})
					break
				case 'imoji':
					reply(mess.wait)
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIO EREN BOT ⚡* 」\n\`\`\``
					no = 0
...
// recursos anime, random @dark domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                ...
                case 'ninjalogo':
                    ...
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '*Gaarazinho⚡*'})
                    break
                    case 'bruteforcefb':
                ...
                case 'plaq':
					...
					reply('*Aguarde...*')
					buffer = await getBuffer(`https://budenter.sirv.com/Images/plaq.jpg?w=800&h=500&text.0.text=${teks}&text.0.position.gravity=center&text.0.position.x=16%25&text.0.position.y=11%25&text.0.size=0&text.0.color=b000ff&text.0.opacity=50&text.0.font.family=Paytone%20One&text.0.font.style=italic`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Sua plaquinha foi feita.*'})
					break
				case 'galaxtext':
					if (args.length < 1) return reply('o que você quer tio')
					teks = body.slice(12)
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'owner':
                    client.sendMessage(from, {displayname: "Gaarazinho", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'f':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(220,iw)':min'(220,ih)':force_original_aspect_ratio=decrease,fps=15, pad=220:220:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
							.toFormat('webp')
							.save(ran)
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						await ffmpeg(`./${media}`)
							...
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(220,iw)':min'(220,ih)':force_original_aspect_ratio=decrease,fps=15, pad=220:220:-1:-1:color=${args[0]}@0.0, split [a][b]; [a] palettegen=reserve_transparent=off; [b][p] paletteuse`])
							.toFormat('webp')
							.save(ran)*/
					} else {
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, vcs sao fodas ♥️️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
				...
				case 'boanoite':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa noite fml ♥️️'})
					break
                case 'belle':                 
				...
				case 'boanoite':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom noite ♥️️'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/gallery/zFvzl2S`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia ♥️️'})
					break
				case 'termux':
					meme = await kagApi.memes()
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSphjB8ZfQ9OxqNdIPcn7L5JankAMshe180ZQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '➣CRIADOR:* 𝐆𝐀𝐀𝐑𝐀𝐙𝐈𝐍𝐇𝐎 愛\n\n➣WPP: wa.me/559684081026\n\n➣INSTA: @_gaaragzx_\n\n\n➣Espero que goste do BOT⚡\n➣BY:️𝐆𝐀𝐀𝐑𝐀𝐙𝐈𝐍𝐇𝐎 愛'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'nsfwloli':
					...
					loli.getNSFWLoli(async (err, res) => {
						...
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Não mando fotinha para membro comum'})
					})
					break
				case 'lolifig':
					cry = getRandom('.gif')
			...
			reply(mess.wait)
               ...
               case 'marca':
					...
					mentions('*_MARCANDO TODOS_* \n➣'+teks+ '𝐄𝐑𝐄𝐍 𝐁𝐎𝐓', members_id, true)
					break
				case 'limpar':
